...
class LogScaler:
    ...
    def inverse_transform(self, y_scaled):
        y = self.scaler.inverse_transform(y_scaled.reshape(-1, 1))
        if self.apply_log:
            y = 10**y - 1.0
...
def generate_fingerprints(smiles_series):
    ...
    fingerprints = [
        generate_morgan_fps(mols),
        generate_avalon_fps(mols),
        generate_erg_fps(mols),
        compute_rdkit_features(mols)
    ]
